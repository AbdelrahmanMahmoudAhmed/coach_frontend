<template>
  ...
  <div class="pb-16">
    <h1 class="text-center font-bold text-[18px] mb-3 sm:text-[26px] lg:text-[32px]">
      {{ $t("user.user_info") }}
    </h1>
    <div class="">
      <div class="flex flex-col justify-center items-center gap-8 lg:mt-[50px] lg:gap-16">
        <div>
          <div v-if="adminName" class="text-[12px] flex gap-2 xs:text-[16px] sm:text-[22px] sm:gap-4">
            <span class="text-[#0000007d] text-nowrap dark:text-[#ffffff82]">{{
        $t("auth.name")
      }}</span><span class="">{{ adminName }}</span>
          </div>

          <div v-if="adminEmail" class="text-[12px] flex gap-2 xs:text-[16px] sm:text-[22px] sm:gap-4">
            <span class="text-[#0000007d] text-nowrap dark:text-[#ffffff82]">{{
        $t("auth.email")
      }}</span><span class="">{{ adminEmail }}</span>
          </div>
        ...
        <div class="w-full">
          <h1 class="text-center font-bold text-[18px] mb-6 sm:text-[26px] lg:text-[32px]">
            {{ $t("user.change_info") }}
          </h1>
          ...
          <form @submit.prevent="updateAdminData" class="w-full">
            ...
            <div class="mb-[10px] w-full flex flex-col justify-center max-w-[700px] m-auto">
              <div class="relative">
                <div @click="() => changeInputType('password')"
                  :class="` ${locale == 'ar' ? ' left-5' : ' right-5'} cursor-pointer absolute top-[15px] `">
                  <img v-if="errors.password.isVisible" src="../../assets/imgs/hide.svg" alt="">
                  <img v-else src="../../assets/imgs/unhide.svg" alt="">

                </div>
                <input v-model="state.password" :placeholder="$t('auth.new_password')"
            ...
            <div class="mb-[10px] w-full flex flex-col justify-center max-w-[700px] m-auto">
              <div class="relative">
                <div @click="() => changeInputType('passwordConfirmation')"
                  :class="` ${locale == 'ar' ? ' left-5' : ' right-5'} cursor-pointer absolute top-[15px] `">
...
const { getCurrentAdmin } = useServerRequest()
