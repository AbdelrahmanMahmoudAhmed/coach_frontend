<template>
  ...
  import { required, minLength, maxLength, requiredIf, helpers , url } from "@vuelidate/validators";
  import useRequest from "~/composables/useRequest";
  import showToast from "~/composables/useToast";
  
  const config = useRuntimeConfig();
  const BASE_URL = config.public.base_url;
  const { createSection, editSection, deleteSection } =
    useRequest();
  ...
  const rules = computed(() => {
    return {
     ...
     nameEn: {
      ...
      callToActionLink:{
        required: helpers.withMessage("add_link", requiredIf(()=>{
    ...
    if (props.type != "delete") {
      ...
      if (result ) {
        ...
        try {
          ...
          if (res.data.success) {
            props.type == "add" &&
              showToast({ message: t("toast.success_add_section") });
            props.type == "edit" &&
              showToast({ message: t("toast.success_edit_section") });
            getSectionsData();
            closePopup();
