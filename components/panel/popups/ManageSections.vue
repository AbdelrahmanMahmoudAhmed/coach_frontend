<template>
  <div @click.self="closePopup" class="popup-holder popup">
    ...
    <form @submit.prevent="manageSectionFun" class="panel-form">
      <div
        @click="closePopup"
      ...
      <div v-if="type != 'delete'" class="flex flex-col justify-center">
        <div class="add-edit-holder" v-if="imageDisplaying">
          <div class="max-w-[200px] rounded-lg overflow-hidden">
            <img :src="imageDisplaying" alt="section image" />
          </div>
        </div>
        <div class="mb-[10px] w-full flex flex-col justify-center max-w-[700px] m-auto">
          <div class="relative">
            <label class="px-[30px] mb-1" for="">{{ $t("auth.section_img") }}</label>
            <input
        ...
        <div class="panel-input-holder">
          <div class="relative">
            <input
              ...
              :placeholder="$t('auth.name_ar')"
              class="panel-input"
              type="text"
            />
          </div>
          <p v-if="errors.nameAr.state" class="panel-input-err">
            {{ $t(`auth.errors.${errors.nameAr.message}`) }}
          </p>
        </div>
        <div class="panel-input-holder">
          <div class="relative">
            <input
              ...
              :placeholder="$t('auth.name_en')"
              class="panel-input"
              type="text"
            />
          </div>
          <p v-if="errors.nameEn.state" class="panel-input-err">
            {{ $t(`auth.errors.${errors.nameEn.message}`) }}
          </p>
        </div>
        <div class="panel-input-holder">
          <div class="relative">
        ...
        <div :class="`panel-input-holder `">
          <div class="flex justify-center gap-6 items-center">
            <label class="switch">
              <input
                type="checkbox"
                @click="() => (state.callToAction = !state.callToAction)"
                :checked="state.callToAction"
              />
              <span class="slider round"></span>
            </label>

            <span>{{ t("auth.has_call_to_action") }}</span>
          </div>
          <div v-if="state.callToAction" class="panel-input-holder mt-4">
            <div class="relative">
              <input
                v-model="state.callToActionLink"
                :placeholder="$t('auth.call_to_action_link')"
                class="panel-input"
                type="text"
              />
            </div>
            <p v-if="errors.callToActionLink.state" class="panel-input-err">
              {{ $t(`auth.errors.${errors.callToActionLink.message}`) }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-else-if="type == 'delete'">
        <div class="text-center font-[900]">
          {{
            $t(`admin.actions.delete_section_msg`, {
              section: locale == "ar" ? currentSection?.titleAr : currentSection?.titleEn,
            })
          }}
        </div>
      </div>
...
<script setup>
const emit = defineEmits(["closePopup", "getSectionsData"]);
...
const { createSection, editSection, deleteSection } = useRequest();
...
const getSectionsData = () => {
  emit("getSectionsData");
};

const manageSectionFun = async () => {
  if (props.type != "delete") {
    errors.nameAr.state = false;
    errors.nameEn.state = false;
    errors.titleAr.state = false;
    errors.titleEn.state = false;
    errors.contentAr.state = false;
    errors.contentEn.state = false;
    errors.callToActionLink.state = false;
    errors.image.state = false;
    ...
    if (result) {
      ...
      state.nameAr && payload.append("nameAr", state.nameAr);
      state.nameEn && payload.append("nameEn", state.nameEn);
      ...
      payload.append("callToAction", state.callToAction);
      state.callToAction &&
        state.callToActionLink &&
        payload.append("callToActionLink", state.callToActionLink);
      state.image && payload.append("image", state.image);
      try {
        let res;
        if (props.type == "add") res = await createSection(payload);
        if (props.type == "edit")
          res = await editSection(props.currentSection?.id, payload);

        if (res.data.success) {
          props.type == "add" && showToast({ message: t("toast.success_add_section") });
          props.type == "edit" && showToast({ message: t("toast.success_edit_section") });
          getSectionsData();
          closePopup();
    ...
    try {
      const res = await deleteSection(props.currentSection?.id);

      if (res.data.success) {
        showToast({ message: t("toast.success_delete_section") });
        getSectionsData();
      } else showToast({ type: "error", message: "err" });
    } catch (err) {
      console.error(err);
...
// hooks
onBeforeMount(() => {
  if (props.type == "edit" && props.currentSection) {
    state.nameAr = props.currentSection?.nameAr;
    state.nameEn = props.currentSection?.nameEn;
    state.titleAr = props.currentSection?.titleAr;
    state.titleEn = props.currentSection?.titleEn;
    state.contentAr = props.currentSection?.contentAr;
    state.contentEn = props.currentSection?.contentEn;
    state.callToAction = props.currentSection?.callToAction;
    state.callToActionLink = props.currentSection?.callToActionLink;
    imageDisplaying.value = `${BASE_URL}${props.currentSection?.image}`;
  }
});
</script>
